...
fn print_information(
    ...
    (0..num_pigs).into_par_iter().for_each(|pig_index| {
        ...
        let mut lookup_table: HashMap<u64, LookUpTableEntry> =
        ...
        let mut sorted_indexes: Vec<&u64> = lookup_table.keys().into_iter().collect();
        sorted_indexes.sort();
...
fn create_show_pigs(
    ...
    let mut banks: Array1<f64> =
        Array1::zeros((test_spins[test_spins.len() - 1] * trials as u64) as usize);
    for fence in fences {
        if !fence.win_type {
            let mut _win_vec: Vec<f64> = Vec::with_capacity(fence.win_dist.len());
            for (key, _win) in &fence.win_dist {
...
fn sort_wins_by_parameter(
    ...
    lookup_table: &mut HashMap<u64, LookUpTableEntry>,
) {
    if fence.win_type {
        let identity_win = fence.identity_condition.win_range_start;
        let keys_to_remove: Vec<u64> = if fence.identity_condition.opposite {
            lookup_table
                .iter()
...
fn parse_fence_info(
    ...
    let win_dist: HashMap<F64Wrapper, Vec<u64>> = HashMap::new();
...
pub struct Fence {
    ...
    pub wins: Vec<f64>,
    pub win_dist: HashMap<F64Wrapper, Vec<u64>>,
...
fn run_simulation(
    ...
    for trial in 0..trials {
        ...
        for (i, &index) in chosen_wins_indices.iter().enumerate() {
            banks_slice[(trial as u64 * spins + i as u64) as usize] = wins[index];
        }
    }
    for trial in 0..trials {
        for (spin_index, &test_spin) in test_spins.iter().enumerate() {
            let bank_slice = &banks.slice(s![
                (trial as usize) * (spins as usize)..((trial as u64 * spins) + test_spin) as usize
            ]);

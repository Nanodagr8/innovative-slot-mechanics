...
window.onBalanceUpdate = (floatAmount) => {
    if (game.running) {
        // Cache server balance to prevent spoiling the outcome and double-adding later
        game.pendingBalance = floatAmount;
    } else {
...
async function finalize(hits, sHitNum, currentBet) {
    ...
    // Apply pending server balance if available (to sync with actual RGS state)
    if (game.pendingBalance !== undefined) {
        game.balance = game.pendingBalance;
        game.pendingBalance = undefined;
    } else {
...
console.log('[HexaKeno] Version 1.0.41 Loaded (Stake Release)');
